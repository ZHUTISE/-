  ...
  return (
    <Card sx={{ mt: 1, width: "100%", maxWidth: 500 }}>
      <CardContent>
        ...
        <Typography sx={{ marginTop: 10 }}>{todo.task}</Typography>
      </CardContent>
      <CardActions>
        <Stack direction="row" spacing={1}>
          {!todo.completed ? (
            <Button onClick={handleCheckboxClick}>Отметить задачу</Button>
          ) : (
            <Button
              variant="contained"
          ...
          <Button onClick={handleRemoveClick}>Закрыть задачу</Button>
        </Stack>
      </CardActions>
    </Card>
  );
